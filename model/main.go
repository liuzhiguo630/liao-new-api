...
func chooseDB(envName string, isLog bool) (*gorm.DB, common.DatabaseType, error) {
	...
	if dsn != "" {
		...
		// liao: PrepareStmt 必须为 false —— LOG_SQL_DSN 可能经 MySQL wire 协议连 ClickHouse，
		// CH 的 MySQL 接口不支持预编译语句；SkipDefaultTransaction 为性能优化。
...
func InitLogDB() (err error) {
	...
	if err == nil {
		common.SetLogDatabaseType(dbType)
		// liao: 允许通过 LOG_DB_TYPE 显式声明日志库类型。
		// ClickHouse 走 MySQL wire 协议时 DSN 形如 MySQL DSN，chooseDB 会误判为 MySQL，
		// 这里在 initCol 之前覆盖，保证列名引用与查询分支走 ClickHouse 语义。
		logDBTypeOverride := common.DatabaseType(strings.ToLower(os.Getenv("LOG_DB_TYPE")))
		if logDBTypeOverride == common.DatabaseTypeClickHouse && dbType != common.DatabaseTypeClickHouse {
		...
		// liao: MySQL wire 协议连 ClickHouse 时日志表由人工管理，
		// 跳过迁移（migrateClickHouseLogDB 的建表/TTL 同步不能作用于既有生产表）。
		if logDBTypeOverride == common.DatabaseTypeClickHouse && dbType != common.DatabaseTypeClickHouse {
			common.SysLog("LOG_DB_TYPE=clickhouse over MySQL wire protocol, skipping log database migration")
			return nil
		}
		common.SysLog("database migration started")
		err = migrateLOGDB()
